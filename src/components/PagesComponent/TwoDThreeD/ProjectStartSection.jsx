      ...
      <div className="container-main">
        ...
        <div className="section-header mb-6 md:mb-10">
          <h2 className="section-title">
            Il Tuo Progetto 3D Inizia Qui.{" "}
            <span className="text-[#8e94f2]">Molto Presto.</span>
          </h2>
          <p className="section-description max-w-[1200px] text-primary-gray/80 mt-4">
            La rivoluzione della stampa 3D è a portata di click. Preparati a
            creare come mai prima d'ora.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12 mt-6 md:mt-10 text-center">
          {steps.map((step, index) => (
            <div key={index} className="flex flex-col items-center gap-4">
              <div className="section-title">{step.value}</div>
              <div className="font-semibold">{step.label}</div>
              <p className="text-primary-gray text-[15px] leading-[22px]">
                {step.description}
              </p>
            </div>
          ))}
        ...
        <div className="mt-10 p-8 bg-[#e5e4f3]  flex items-start gap-5">
          <CheckIcon />
          <p className="text-[15px] leading-[18px] text-primary-black/80 w-[80%]">
            🚀 <b>Lavori in corso:</b> Il servizio è in fase di sviluppo
            avanzato. Iscriviti ora per non perdere il lancio ufficiale e
            accedere alle promozioni riservate ai primi utenti.
          </p>
        </div>
