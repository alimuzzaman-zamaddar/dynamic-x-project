...
export default function UploadNewFile() {
  ...
  const uploadFile = async (file, fileContent) => {
    ...
    try {
      ...
      localStorage.setItem('uploadedModel', JSON.stringify(modelPayload));
  ...
  useEffect(() => {
    ...
    const url = base ? `${base}/quote/technologies` : '/quote/technologies';
    const fetchTech = async () => {
      try {
        ...
        if (Array.isArray(json.data) && json.data.length) {
          ...
          setSelectedTechId((prev) => prev || json.data[0].id);
        }
      } catch (err) {
          ...
          <div className="space-y-6">
            ...
            <div className="space-y-10">
              ...
              >
                ...
                <div
                  className={`bg-gray-100 ${uploading ? 'cursor-not-allowed opacity-70' : 'group-hover:bg-gray-200'} text-gray-800 text-sm font-medium px-5 py-2 rounded-lg border border-gray-300 shadow-sm transition-all`}
                >
                  {uploading ? 'Uploading…' : 'Select from file'}
                </div>
